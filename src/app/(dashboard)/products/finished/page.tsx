...
export default async function FinishedProductsPage() {
  const [{ userRole }, result] = await Promise.all([getAuth(), getProducts('FINISHED')]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage recipes for finished products and their ingredient
            compositions.
          </p>
        </div>
      ...
      <ProductTable data={result.products || []} newHref="/products/finished/new" />
    </div>
  );
}
