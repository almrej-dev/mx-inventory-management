...
import { getRecipeColumns } from "@/components/recipes/recipe-columns";
...
export function RecipeTable({ data }: RecipeTableProps) {
  ...
  const handleDelete = useCallback(
    async (id: number, name: string) => {
      if (!window.confirm(`Delete recipe for "${name}"? This will remove all ingredient associations.`)) return;
      try {
  ...
  const columns = useMemo(
    () => getRecipeColumns({ onDelete: handleDelete }),
    [handleDelete]
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">
          Showing {filteredRowCount} of {data.length} recipes
        </span>
      </div>
